...
ggplot(data = thresholds_splatter) +
  ...
  theme_bw()
ggsave(here('figs','S2A.splatter.adapt.flickerLMS.splatter.pdf'), width = 10, height = 5)
```
# Predict JNDs
...
plot_JNDs(JNDs) +
    ...
    ggtitle("Predicted threshold, if flicker LMS splatter is considered (Adapt)")
ggsave(here('figs','S2B.JNDs.adapt.flickerLMS.splatter.pdf'), width = 10, height = 5)
```