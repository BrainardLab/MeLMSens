...
ggplot(data = backgroundLMS) +
  ...
  theme_bw()
ggsave(here('figs','S1A.splatter.adapt.backgroundLMS.splatter.pdf'), width = 10, height = 5)
```
# Predict JNDs
...
plot_JNDs(JNDs) +
    ...
    ggtitle("Predicted threshold, if background LMS splatter is considered (Adapt)")
ggsave(here('figs','S1B.JNDs.adapt.backgroundLMS.splatter.pdf'), width = 10, height = 5)
```